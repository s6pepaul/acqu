  ...
  :TA2Apparatus( name, fAnalysis, kValidDetectors )
{
  ...
  //app
  fNormEnergyIndex      = 700.0;  // defaults for Ee=855, peak ~ 350;
  fEdgeMinIndex         = 280.0;
  fEdgeMaxIndex         = 420.0;
...
void TA2LinearPolEpics::PostInitialise( )
{
  ...
  case ELpInitLevel1:	//2nd pass after display to get the ptrs to the histograms
    ...
    if(fHEdgeGated!=NULL){
      ...
      fHEdgeGated->SetMinimum(fEdgeMinIndex);
      fHEdgeGated->SetMaximum(fEdgeMaxIndex);
    }
    ...
    if(fHEdgeGatedPerp!=NULL){
      ...
      fHEdgeGatedPerp->SetMinimum(fEdgeMinIndex);
      fHEdgeGatedPerp->SetMaximum(fEdgeMaxIndex);
    ...
    if(fHEdgeGatedPara!=NULL){
      ...
      fHEdgeGatedPara->SetMinimum(fEdgeMinIndex);
      fHEdgeGatedPara->SetMaximum(fEdgeMaxIndex);
...
void TA2LinearPolEpics::Reconstruct( ){
  ...
  if(gAR->IsEpicsRead()||gAR->IsScalerRead()){
	...
	exit(1);
      ...
      for(int index=0;index<appNRunRanges;index++){ //find app table for this run
	...
	if((fHaveIncGatedScaler)&&(fHGatedCurrEnh)){	       	//find the gated coherent edge		 
	  if(fPlane == ETableAmo){
	  ...
	  else
	    {
	      binx=0;
	      ymax=0;
	      for(int b=fHGatedCurrEnh->FindBin(fEdgeMinIndex);b<fHGatedCurrEnh->FindBin(fEdgeMaxIndex);b++){
...
void TA2LinearPolEpics::ParseMisc(char *line){	// read parameters in the setup file
  ...
  switch(type){
  case ELpMiscApp:
    if(sscanf(line, "%*s%d%d%lf%lf%lf%s%s%s%d%s%s%lf",&appRunRangeMin[appNRunRanges],&appRunRangeMax[appNRunRanges],&fNormEnergy[appNRunRanges],&fEdgeMin[appNRunRanges],&fEdgeMax[appNRunRanges],fTaggerName,
	      fLadderName,fRunRefFiles[appNRunRanges],&fNScalerBuffers,fEdgeString,fPlaneString,&fDeadband)!=12){
